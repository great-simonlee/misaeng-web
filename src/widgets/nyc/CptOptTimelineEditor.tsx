  ...
  return (
    <div className={cn('space-y-3', className)}>
      <GuideBox
        tone='create'
        title='시작점만 남겨 주세요'
        description='오늘은 첫 날짜와 진행 상황만 적으면 충분해요. 몇 주·몇 달 뒤의 기록은 업데이트에서 한 건씩 이어 가면 됩니다.'
      />
      <QuickStepButtons
        cptOptType={cptOptType}
  ...
  return (
    <div className={cn('space-y-4', className)}>
      <GuideBox
        ...
        description={
          isEditing
            ? '아래 폼에서 한 건만 수정한 뒤, 화면 아래 업데이트 버튼으로 저장해 주세요.'
            : '새 날짜와 진행 상황만 적으면 됩니다. 기존 기록은 아래에서 참고하거나 수정할 수 있어요.'
        }
      />
...
function TimelineField({
  ...
  return (
    <label className={cn('block rounded-xl px-3 py-2.5', field.rowClass)}>
      <span className={cn('text-[12px] font-semibold', field.labelClass)}>
        {field.label}
      </span>
