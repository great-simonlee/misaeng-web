...
export function CptOptWriteScreen({
  ...
  const boardId: NycCommunityBoardId = 'cpt-opt'
  ...
  const [cptOptTips, setCptOptTips] = useState('')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        if (!post || post.categoryId !== boardId) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!cptOptType) {
      toastError('CPT / OPT / STEM OPT 중 유형을 선택해 주세요')
      return
    }
    ...
    try {
      const payload = {
        ...
        cptOptTips: cptOptTips.trim() || null,
      }
          ...
          {isEdit ? (
              ...
              <section className='overflow-hidden rounded-2xl ring-1 ring-black/[0.06]'>
                ...
                >
                  <div>
                    ...
                    <p className='mt-0.5 text-[11px] text-[var(--muted)]'>
                      제목, 조심할 점, 후기 본문을 바꾸고 싶을 때만 열어 주세요
                    </p>
                  </div>
                ...
                {showMoreSettings ? (
                  <div className='space-y-5 border-t border-black/[0.04] bg-white p-4 sm:p-5'>
                    ...
                    <Field label='조심해야 할 점'>
                      <textarea
                        value={cptOptTips}
                        onChange={(e) =>
                          setCptOptTips(
                            e.target.value.slice(0, CPT_OPT_TIPS_MAX),
                          )
                        }
                        maxLength={CPT_OPT_TIPS_MAX}
                        rows={3}
                        placeholder='새로 알게 된 주의사항이 있으면 적어 주세요'
                        className={`${inputClass} min-h-[5.5rem] resize-none py-3`}
                      />
                    </Field>
                    <div>
            ...
            <>
              <CreateSection
                step={1}
                title='어떤 유형인가요?'
                description='CPT / OPT / STEM OPT 중 해당하는 것을 선택해 주세요'
              >
                <CptOptTypePicker
                  value={cptOptType}
              ...
              <CreateSection
                step={3}
                title='첫 진행 기록'
                description='지금 시점의 날짜와 진행 상황만 적어 주세요. 나중에 이어서 추가할 수 있어요'
              >
                <CptOptTimelineEditor
                  value={timeline}
                  onChange={setTimeline}
              ...
              >
                <textarea
                  value={cptOptTips}
                  onChange={(e) =>
                    setCptOptTips(e.target.value.slice(0, CPT_OPT_TIPS_MAX))
                  }
                  maxLength={CPT_OPT_TIPS_MAX}
                  rows={4}
                  placeholder='예: 회사 시작일 2–3주 전에 학교에 서류를 넣으세요. 학교마다 포털이 달라 ISS 체크리스트를 먼저 확인하세요.'
                  className={`${inputClass} min-h-[6.5rem] resize-none py-3`}
                />
                <p className='mt-1 text-right text-[10px] tabular-nums text-[var(--muted)]'>
                  {cptOptTips.length}/{CPT_OPT_TIPS_MAX}
                </p>
              </CreateSection>
              ...
              <CreateSection
                step={5}
                title='후기 본문'
                description='전체 경험, 학교·USCIS와의 소통, 기타 참고할 내용'
              >
                <TipTapEditor
                  value={contentHtml}
                  onChange={setContentHtml}
  ...
  return (
    <div className='rounded-2xl bg-[#f7f8fa] px-4 py-5 ring-1 ring-black/[0.04] sm:px-5'>
      ...
      <p className='mt-2 text-[13px] leading-relaxed text-[var(--muted)]'>
        유형·기본 정보·첫 진행 기록을 한 번에 세팅하는 화면이에요. 이후
        진행 상황은 업데이트에서 한 건씩 추가하면 됩니다.
      </p>
    </div>
  )
}
